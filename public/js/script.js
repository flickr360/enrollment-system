// Function to toggle dropdown visibility and show courses on the schedule
    ...
    course.addEventListener('click', function () {
        ...
        days.forEach(day => {
            ...
            for (let row = startRow; row <= endRow; row++) {
                ...
                if (slot.querySelector(`[data-id="${courseId}"]`)) {
                    courseAlreadyAdded = true;
                    break;
                }
            }

            if (courseAlreadyAdded) return;
        });

        if (courseAlreadyAdded) {
            alert('Warning: This course conflicts with your existing schedule!');
            return;
        }
